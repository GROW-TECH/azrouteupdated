...
export async function GET() {
  try {
    ...
    const authToken = cookieStore.get("auth-token") || cookieStore.get("coach-token");
    if (!authToken?.value) return NextResponse.json({ message: "Not authenticated" }, { status: 401 });
    ...
    if (role === "coach" || role === "teacher") {
      // select only columns that exist in coaches table
