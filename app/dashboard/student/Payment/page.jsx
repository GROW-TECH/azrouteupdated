...
/**
 * PaymentPage
 * - If student.status === "Paid" -> show only payment history in a neat style
 * - If status === "Pending" -> show original view with Pay Now button + modal
 * - Razorpay flow kept intact
 * - Display-friendly method names: hide raw "Razorpay" label and show "Online"
 */
...
export default function PaymentPage() {
  ...
  // Payment modal state (will be disabled if Paid)
  const [showPayModal, setShowPayModal] = useState(false);
  const [payAmount, setPayAmount] = useState("");
  const [payMethod, setPayMethod] = useState("UPI");
  const [processing, setProcessing] = useState(false);

  // refs for realtime or cleanup (not used here)
    ...
    const load = async () => {
      try {
        ...
        const { data: studentRow, error: studentErr } = await supabase
          .from("student_list")
          .select("id, reg_no, name, email, course, payment, status, fees, phone")
          .eq("email", email)
          .single();
  ...
  const openPayModal = () => {
    ...
    // If already paid, do not allow opening modal
        ...
        handler: async function (response) {
          try {
            ...
            const { data: refreshedStudent } = await supabase
              .from("student_list")
              .select("id, reg_no, name, email, course, payment, status, fees")
              .eq("id", student.id)
              .single();
  ...
  return (
    <div className="space-y-8 p-6">
        ...
        <Card className="border shadow-sm">
          ...
          <CardContent className="text-sm space-y-2 text-muted-foreground">
            ...
            {summaryStatus === "Pending" && (
              <Button className="mt-3 bg-[#3b82f6] hover:bg-[#2563eb]" onClick={openPayModal}>
                Pay Now
              </Button>
            )}
          </CardContent>
      ...
      {/* modal (only possible when not paid) */}
      {showPayModal && summaryStatus !== "Paid" && (
